...
def get_ipaddr(url='http://httpbin.org/ip', proxy=None):
    ...
    # print("get_ipaddr... proxy:", proxy)
    if proxy:
        proxy = '{0}'.format(proxy)
        proxies = {'http': proxy, 'https': proxy}
        # print("get_ipaddr... proxies:", proxies)
        try:
            response = requests.get(url, proxies=proxies)
        except:
            return None
        # print("get_ipaddr... response:", response)
    else:
...
def get_cli_args():
    ...
    main.add_argument(
        '--file',
        help='Youtube URL file list (one url per line)'
    )
    main.add_argument(
        '--proxy',
...
async def save(proxies):
    while True:
        ...
        proxyList.append('%s://%s:%d' % (proto, proxy.host, proxy.port))

def get_broker_proxies():
